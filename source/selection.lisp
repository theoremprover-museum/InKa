...
(defvar sel*interactive 

  ;;; a flag indicating whether the proof sketch should be presented at the end of 
  ;;; the proof attempt
...
(defun sel-set.interactive (flag)
  
  (setq sel*interactive flag))
...
(DEFUN SEL=THEOREM.PROVE (THEOREM QUIET FILE)
  ...
  (let ((neg.theorem (NORM-NORMALIZATION (DA-FORMULA.NEGATE (da-gterm.copy THEOREM))))
    ...
    (COND ((not QUIET)
	   ...
	   (cond ((or sel*interactive (not (sel=case.is.proved case)))
		  (pr-graphic.interact 
  ...
  (let* ((vars (smapcar #'(lambda (x) x) #'cddr (car termsubsts)))
    ...
    (cond (substs
	   (multiple-value-bind (residuum new.substs)
	       (sel=ind.compute.resolvent (list conclusion) (list hypothesis) substs (cdr adv.hypos))
	     (cond (new.substs (mapcar #'(lambda (form)
					   (uni-subst.apply (car new.substs) form nil 'induction 'induction))
				       residuum))))))))


(defun sel=ind.compute.resolvent (conclusion hypothesis substs alt)
  ...
  (let (add.concl add.hypos residuum total.residuum)
    (cond ((da-literal.is (car hypothesis))
	   (sel=ind.compute.single.resolvent conclusion hypothesis substs alt))
...
(defun sel=ind.compute.single.resolvent (concl hypo substs alt)

  (let (new.concl (subtaf.descrs (getf (da-gterm.attributes (car concl)) 'induction))
		  (successes (getf (da-literal.attributes (car hypo)) 'successes)))
    (setq new.concl (da-gterm.copy (car concl)))
    (mapc #'(lambda (subtaf.descr)
	      (cond ((sel=ind.subtaf.descr.is.successful subtaf.descr (car concl) successes)
	  ...
	  subtaf.descrs)
    (setq subtaf.descrs (remove-if #'(lambda (subtaf.descr)
				       (not (sel=ind.subtask.is.open new.concl (third subtaf.descr))))
				   subtaf.descrs))
    (cond (subtaf.descrs
	   (setf (getf (da-gterm.attributes new.concl) 'id) (getf (da-gterm.attributes (car concl)) 'id))
	   (setf (getf (da-gterm.attributes new.concl) 'induction) subtaf.descrs)
	   (setf (getf (da-gterm.attributes new.concl) 'taf) (getf (da-gterm.attributes (car concl)) 'taf))
	   (sel=ind.search.for.alternative.descr (cons new.concl (cdr concl)) hypo substs subtaf.descrs alt))
	  (t (values (list (sel=ind.subtask.simplify
			    (da-formula.junction.closure 'and (cons new.concl (cdr concl)))))
		     substs)))))


(defun sel=ind.search.for.alternative.descr (concl hypos substs subtaf.descrs alts)

  (let ((taf (car (getf (da-gterm.attributes (car concl)) 'taf)))
	 new.descr remaining.alts new.substs tasks other.tasks)
    (cond ((setq subtaf.descr 
		 (find-if #'(lambda (subtaf.descr)
			      (setq new.descr 
				    (find-if #'(lambda (hypo.subst.subtaf.descr)
						 (and (setq new.substs (uni-subst.list.merge 
									(second hypo.subst.subtaf.descr) 
									substs))
						      (sel=ind.subtaf.descr.is.successful
						       subtaf.descr (car concl)
						       (third hypo.subst.subtaf.descr))))
					     alts)))
			  subtaf.descrs))
	   (setq remaining.alts (remove new.descr alts))
	   (multiple-value-setq (tasks new.substs)
	     (sel=ind.compute.single.resolvent concl (cons (da-access taf (car new.descr)) hypos) new.substs remaining.alts))
	   (setq tasks (append tasks
			       (mapcan #'(lambda (other.hypo)
					   (multiple-value-setq (other.tasks new.substs)
					     (sel=ind.search.for.alternative.concl
					      concl (append (da-formula.junction.open 'and other.hypo) hypos)
					      new.substs remaining.alts))
					   other.tasks)
				       (da-formula.junction.open 'or (DA-GTERM.WITHOUT.TAFS (car new.descr) taf)))))
	   (values tasks new.substs))
	  (t (values (list (sel=ind.subtask.simplify
			    (da-formula.junction.closure 'and (cons (da-gterm.colourize (car hypos) 'induction 'hypo) concl))))
		     substs)))))
  


(defun sel=ind.search.for.alternative.concl (concl hypo substs remaining.alts)

  (let (new.conc (successes (getf (da-gterm.attributes (car hypo)) 'successes)))
    (cond ((setq new.conc (find-if #'(lambda (concl.part)
				       (some #'(lambda (subtaf.descr)
						 (sel=ind.subtaf.descr.is.successful subtaf.descr concl.part successes))
					     (getf (da-gterm.attributes concl.part) 'induction)))
				 concl))
	   (sel=ind.compute.single.resolvent (cons new.conc (remove new.conc concl))
					     hypo substs remaining.alts))
	  (t (sel=ind.search.for.alternative.descr concl hypo substs (getf (da-gterm.attributes (car concl)) 'induction)
						   remaining.alts)))))
...
(defun sel=ind.subtask.simplify (gterm)

  (cond ((member (da-gterm.symbol gterm) '(and or))
	 (let ((termlist (mapcar #'(lambda (subterm)
				     (sel=ind.subtask.simplify subterm))
				 (da-gterm.termlist gterm))))
	   (cond ((eq 'and (da-gterm.symbol gterm))
		  (cond ((some #'(lambda (subterm) (da-formula.is.false subterm)) termlist)
			 (da-literal.false))
			(t (da-formula.junction.closure 'and termlist))))
		 (t (da-formula.junction.closure 
		     'or (remove-if #'(lambda (subterm) (da-formula.is.false subterm)) termlist))))))
	((and (da-literal.is gterm)
	      (da-literal.is.negated.equation gterm)
	      (uni-term.are.equal (car (da-literal.termlist gterm)) (second (da-literal.termlist gterm))))
	 (da-literal.false))
	(t gterm)))
...
(defun sel=ind.replace.subterm (concl hypo subtaf.descr)
  ...
  (cond ((eq (car subtaf.descr) 'res)
	...
	(t (da-replace (append (fifth subtaf.descr) (fourth subtaf.descr) (third subtaf.descr))
		       concl (da-gterm.colourize
			      (da-gterm.copy (da-access (da-taf.otherside (fourth subtaf.descr)) hypo))
			      'hypo 'induction)))))


(defun sel=ind.subtaf.descr.is.successful (subtaf.descr gterm subtaf.descrs)

  (some #'(lambda (id.taf.subtaf)
	    (and (eq (car id.taf.subtaf) (getf (da-gterm.attributes gterm) 'id))
		 (equal (third id.taf.subtaf) subtaf.descr)))
	subtaf.descrs))
...
(defun sel=ind.mark.correspondent.literals (conclusion hypotheses vars)
  ...
  (let ((tafs (da-gterm.literals.with.property (car hypotheses) #'(lambda (lit) lit)))
	total.unifiers adv.hypotheses sucesses concl.lit hypo.lit unifiers success)
	      ...
	      (mapc #'(lambda (taf)
			...
			(mapc #'(lambda (subtaf.descr)
				  ...
				  (cond (unifiers (setq total.unifiers (uni-subst.list.merge
						  ...
						  (setq success (list (getf (da-gterm.attributes concl.lit) 'id)
								      taf subtaf.descr))
						  (push success (getf (da-literal.attributes hypo.lit) 'successes))
						  (push success sucesses))))
...
(defun sel=ind.concl.insert.ind.terms (conclusion tafs vars)
  ...
  (let ((counter 0))
    (mapc #'(lambda (taf)
		...
		(setf (getf (da-gterm.attributes concl.subformula) 'id) (incf counter))
...
(defun sel=ind.gterm.choose.equation.side (equation ind.vars)
  ...
  (let* ((ex.vars (da-gterm.variables equation))
	 ...
	 (lcons (da-gterm.constants left 'skolem))
	 (rcons (da-gterm.constants right 'skolem)))
    (cond ((not (da-gterm.tafs left #'(lambda (term) (member term ind.vars :test 'uni-gterm.are.equal))))
	  ...
	  ((and ex.vars (not (intersection (da-gterm.variables left) ex.vars)))
	   (da-taf.create.left))
	  ((and (subsetp lcons rcons) (not (subsetp rcons lcons)))
	   (da-taf.create.right))
	  ((and (subsetp rcons lcons) (not (subsetp lcons rcons)))
	   (da-taf.create.left))
...
(defun sel=context.insert (occ taf wished.terms colour.key solution.colour failure.colour &optional sink.colour)
  ...
  (let ((org.colour (da-gterm.colour (da-access taf (rl-object occ)) colour.key))
	used.wished.terms new.wished.terms tafs actual.expansion wished.tafs sel.taf)
...
(defun sel=man.case.interaction.leaf (case)
  ...
  (case (car (WIN-MN.POPUP (win-window 'description_1)
	...
	(5 (sel=mod.check.counter.example case))
...
(defun sel=mod.check.counter.example (case)

  (let ((object (sel=object.simplified.formula (sel=case.proof case)))
	(condition (sel=case.condition case)) result used.defs eval.list)
    (multiple-value-setq (result used.defs eval.list)
      (sat-satisfy.neg.gterm (da-formula.junction.closure
			      'and (cons (sel=object.gterm object) condition))))
    (pr-graphic.interact (win-window 'description_3)
			 `(sel=pr.present.counter.example (quote ,(sel=object.gterm object)) (quote ,condition)
							  (quote ,result) (quote ,used.defs) (quote ,eval.list))))
  nil)
...
(defun sel=pr.present.counter.example (gterm condition result used.defs eval.list)

  (pr-parse.headline "Checking for counterexamples"
		     (pr-parse.itemize
		      (list (cond (result (pr-parse.string "Example found to satisfy" :colour "DarkBlue"))
				  (t (pr-parse.string "No example found to satisfy" :colour "DarkBlue")))
			    (pr-parse.gterm gterm 200)
			    (pr-parse.string "under conditions" :colour "DarkBlue")
			    (pr-parse.gterm (da-formula.junction.closure 'and condition) nil)
			    (cond (result (pr-parse.itemize 
					   (list (pr-parse.string "Consider the following instantiations:" :colour "DarkBlue")
						 (pr-parse.table
						  (append (mapcar #'(lambda (var.term)
								      (pr-parse.string 
								       (format nil "~A :" (da-function.pname (car var.term)))))
								  result)
							  (mapcar #'(lambda (var.term)
								      (pr-parse.gterm (cdr var.term) 200))
								  result))
						  2)
						 (pr-parse.string "")
						 (pr-parse.FLOAT.TEXT
						  (cons (pr-parse.string "using the definition(s) of " :colour "DarkBlue")
									 (mapcar #'(lambda (x) (pr-parse.string
												(format nil "~A " (da-object.pname x))))
										 used.defs)))
						 (pr-parse.closure 
						  (pr-parse.string "Illustrate" :colour "Red"
								   :double.left `(sel=mod.counter.example
										  (quote ,eval.list) (quote ,(1- (length eval.list)))))
						  'boxed 2 "LightGrey" 3 :font 'bold :size 'D))
...
(defun sel=mod.counter.example (eval.list index)

  (let ((tafs (cond ((> index 0) (car (nth (1- index) eval.list))))))
    (pr-parse.headline "Counterexample"
		       (pr-parse.itemize
			(list (pr-parse.float.text
			       (list (cond ((> index 1)
					    (pr-parse.closure (pr-parse.string "Next step" :colour "Red" :double.left
									       `(sel=mod.counter.example (quote ,eval.list) (quote ,(1- index))))
							      'boxed 2 "LightGrey" 3 :font 'bold :size 'D))
					   (t (pr-parse.string "        ")))
				     (pr-parse.string "                                                              ")
				     (cond ((< index (1- (length eval.list)))
					    (pr-parse.closure (pr-parse.string "Prev.step" :colour "Red" :double.left
									       `(sel=mod.counter.example (quote ,eval.list) (quote ,(1+ index))))
							    'boxed 2 "LightGrey" 3 :font 'bold :size 'D))
					   (t (pr-parse.string " ")))))
			      (pr-parse.gterm (cdr (nth index eval.list)) 200 
					      #'(lambda (subterm taf) (cond ((some #'(lambda (taf2) (da-taf.is.subtaf taf2 taf)) tafs) (list :colour "Red"))
								    (t (list :colour "Black")))))
			      (cond ((> index 0) (pr-parse.string "evaluates to" :colour "DarkBlue"))
				    (t (pr-parse.string " ")))
			      (cond ((> index 0) (pr-parse.gterm (cdr (nth (1- index) eval.list)) 200
								 #'(lambda (subterm taf) (cond ((some #'(lambda (taf2) (da-taf.is.subtaf taf2 taf)) tafs)
												(list :colour "Red"))
											       (t (list :colour "Black"))))))
				    (t (pr-parse.string "No further modifications available"))))
			nil :font 'bold :size 'D))))
