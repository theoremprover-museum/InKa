...
(DEFUN NORM-NORMALIZE.GTERM (GTERM &optional negate)
  ...
  (COND ((DA-LITERAL.IS GTERM) 
	 (cond (negate (da-formula.negate GTERM))
	       (t gterm)))
	((Eq (DA-GTERM.SYMBOL GTERM) 'not) 
	 (NORM-NORMALIZE.GTERM (car (da-gterm.termlist gterm)) (not negate)))
	((member (DA-GTERM.SYMBOL GTERM) '(OR AND))
	 (let ((symbol (cond (negate (cond ((eq (DA-GTERM.SYMBOL GTERM) 'and) 'or) (t 'and)))
			     (t (DA-GTERM.SYMBOL GTERM)))))
	   (norm=junction.closure symbol
				  (MAPCAN #'(LAMBDA (SUB.TERM)
					      (DELETE-IF #'(LAMBDA (X) (cond ((eq symbol 'and) (DA-FORMULA.IS.TRUE X))
									     (t (DA-FORMULA.IS.FALSE X))))
							 (DA-FORMULA.JUNCTION.OPEN symbol (NORM-NORMALIZE.GTERM SUB.TERM negate))))
					  (DA-GTERM.TERMLIST GTERM)))))))
