...
(declaim (TYPE FIXNUM win*max.buffer))
...
(defvar win*input.stream nil)

(defvar win*output.stream nil)
...
(defvar win*window.all (let ((counter -1))
			 (mapcar #'(lambda (name from.name tcl.name)
				     (make-win*window :name name :tcl.name tcl.name :from.name from.name
						      :control.index (incf counter)
						      :request.index (incf counter) :colours (list "Black")))
				 '(description_1 description_2 description_3 proof main)
				 '(.descr1 .descr2 .descr3 .proto .main)
				 '(".descr1" ".descr2" ".descr3" ".proto" ".main"))))


(defvar win*error nil)

(declaim (TYPE (SIMPLE-ARRAY FIXNUM 1) win*cursors win*ends))

(declaim (TYPE (SIMPLE-ARRAY * 2) win*buffers))

(declaim (TYPE LIST win*window.all))

(declaim (TYPE STREAM win*input.stream win*output.stream win*error.stream))

(declaim (TYPE (OR  NIL T) win*active))
...
(defun win-reset (&optional type)
  ...
  ;;;          The C-process is always called with at least one argument: 
  ;;;          the path of the idm directory,
  ...
  (declare (ignore type))
  (let* ((directory (append (pathname-directory *inka-pathname*) (list "tcl")))
	 (name (namestring (make-pathname :name *inka-tk-wish*))))
    (ps-reset)
    (multiple-value-setq (win*output.stream win*input.stream win*error)
      (pro-run.foreign.process name :stream :stream :stream nil 
			       (cond ((pro-environment-variable "INKA-COLORMAP")
				      (list "-colormap" "new")))))
    (format win*output.stream "source ~A~%" (namestring (make-pathname :directory directory :name "init.tcl")))
    (force-output win*output.stream)
    (setq win*bitmaps nil)
    (setq win*stream.process (pro-process.create :name "IO-Listener" 
						 :function #'win=handle.input.stream))))
...
(defun win=handle.input.stream ()
  ...
  (let* ((*package* (find-package 'inka)) pipe pipe.win new.entry (pipe.no 0) data)
    (declare (type fixnum pipe.no))
    (sleep 1)
    (catch 'win*io.abort
      (win=search.for.init.string)
      (loop
	 (setq pipe (read win*input.stream nil 'win*io.fail))
	 (cond ((setq pipe.win (find-if #'(lambda (window) (equal (car pipe) (win=window.from.name window))) win*window.all))
		(setq pipe.no (cond ((eq (second pipe) 'c) (win=window.control.index pipe.win))
				    (t (win=window.request.index pipe.win))))
		(cond ((neq 'check.aborted (setq data (win=io.check.keys (third pipe))))
		       (setf (aref win*buffers pipe.no (aref win*ends pipe.no)) data)
...
(defun win=search.for.init.string ()

  (while (not (eql #\O (read-char win*input.stream))))
  (cond ((not (eql #\K (read-char win*input.stream)))
	 (win=search.for.init.string))))


(defun win=io.check.keys (data)

  (cond ((and (consp data) (eq (car data) 'key))
	 (let ((key (second data)) new.key)
	   (cond ((> key 512)
		  (setq new.key (cond ((equalp key 65288) #\Backspace)
				      ((equalp key 65535) #\Rubout)
				      ((and (> key 65469) (< key 65482)) (intern (format nil "F~D" (- key 65469))))
				      ((or (equalp key 65362) (equalp key 65431)) 'Up)
				      ((or (equalp key 65361) (equalp key 65430)) 'Left)
				      ((or (equalp key 65363) (equalp key 65433)) 'Right)
				      ((or (equalp key 65364) (equalp key 65432)) 'Down)
				      ((equalp key 65360) 'home)
				      ((equalp key 65365) 'pageup)
				      ((equalp key 65366) 'pagedown)
				      ((equalp key 65307) #\ESC)))
		  (cond (new.key (setf (second data) new.key)
				 data)
			(t 'check.aborted)))
		 (t (setf (second data) (code-char (second data)))
		    data))))
	(t data)))
...
(defun win-io.draw.bitmap (window width height x-pos y-pos bitmap.file)
  ...
  (cond ((eq (type-of window) 'ps*postscript.handler)
	...
	(t (let (bitmap.name)
	     (setq bitmap.name (win=bitmap.name (format nil "~A~D" bitmap.file (win=bitmap.scale width))))
	     (pro-without.scheduling
	      (format win*output.stream
		      "~A.canvas create image ~D ~D -image ~A -anchor nw ~%" 
		      (win=window.tcl.name window) x-pos y-pos
		      bitmap.name))
	     (cond ((not win*forced.output.disabled) (force-output win*output.stream)))))))
...
(defun win-io.draw.rectangle (window width height x-pos y-pos &optional (line.width 1) colour)
  ...
  (cond ((eq (type-of window) 'ps*postscript.handler)
	...
	(t (cond ((null colour) (setq colour "")))
	   (pro-without.scheduling
	    (format win*output.stream 
		    "~A.canvas create rectangle ~D ~D ~D ~D -width ~D -outline ~s -fill ~s~%" 
		    (win=window.tcl.name window) x-pos y-pos (+ x-pos width) (+ y-pos height) line.width 
		    (win=colour.to.tcl (car (win=window.colours window)))
		    (win=colour.to.tcl colour)))
	   (cond ((not win*forced.output.disabled) (force-output win*output.stream))))))
...
(defun win-io.invert (window width height x-pos y-pos)
  ...
  (pro-without.scheduling
	    (format win*output.stream 
		    "inverse ~A ~D ~D ~D ~D~%" 
		    (win=window.tcl.name window) x-pos y-pos (+ x-pos width) (+ y-pos height)))
  (cond ((not win*forced.output.disabled) (force-output win*output.stream))))
...
(defun win-io.draw.circle (window radius x-pos y-pos &optional (line.width 1) colour)
  ...
  (cond ((eq (type-of window) 'ps*postscript.handler)
	...
	(t (cond ((null colour) (setq colour "Black")))
	   (pro-without.scheduling
	    (format win*output.stream 
		    "~A.canvas create oval ~D ~D ~D ~D -width ~D -outline ~s -fill ~s~%" 
		    (win=window.tcl.name window) (- x-pos radius) (- y-pos radius) (+ x-pos radius) 
		    (+ y-pos radius) line.width
		    (win=colour.to.tcl (car (win=window.colours window)))
		    (win=colour.to.tcl colour)))
	   (cond ((not win*forced.output.disabled) (force-output win*output.stream))))))
...
(defun win-io.draw.oval (window x-pos y-pos width height &optional (line.width 1) colour)
  ...
  (cond ((eq (type-of window) 'ps*postscript.handler)
	...
	(t (cond ((null colour) (setq colour "")))
	   (pro-without.scheduling
	    (format win*output.stream "~A.canvas create oval ~D ~D ~D ~D -width ~D -outline ~s~%" 
		    (win=window.tcl.name window) x-pos y-pos (+ x-pos width) (+ y-pos height) 
		    line.width (win=colour.to.tcl colour)))
	   (cond ((not win*forced.output.disabled) (force-output win*output.stream))))))
...
(defun win-io.draw.line (window from-x from-y to-x to-y &optional (line.width 1))
  ...
  (cond ((eq (type-of window) 'ps*postscript.handler)
	...
	(t (pro-without.scheduling
	    (format win*output.stream "~A.canvas create line ~D ~D ~D ~D -width ~D -fill ~s~%" 
		    (win=window.tcl.name window) from-x from-y to-x to-y line.width
		    (win=colour.to.tcl (car (win=window.colours window)))))
	   (cond ((not win*forced.output.disabled) (force-output win*output.stream))))))
...
(defun win-io.draw.string (window x-pos y-pos string)
  ...
  (cond ((eq (type-of window) 'ps*postscript.handler)
	...
	(t (pro-without.scheduling
	    (format win*output.stream "~A.canvas create text ~D ~D -text "
		    (win=window.tcl.name window) x-pos y-pos)
	    (win=io.submit.string string)
	    (format win*output.stream " -font ~s -fill ~s -anchor nw~%"
		    (win=determine.font (win=window.font window))
		    (win=colour.to.tcl (car (win=window.colours window)))))
	   (cond ((not win*forced.output.disabled) (force-output win*output.stream))))))



(defun win=io.submit.string (string &optional newline)

  (let ((length (length string)) char)
    (write-char #\" win*output.stream)
    (do ((cursor 0 (1+ cursor)))
	((eq cursor length))
	(setq char (aref string cursor))
	(cond ((member char '(#\" #\{ #\[ #\] #\} #\" #\\))
	       (write-char #\\ win*output.stream)))
	(write-char (aref string cursor) win*output.stream))
    (cond (newline (terpri win*output.stream)))
    (write-char #\" win*output.stream)))
	 ...
	 (values (if (equal (ps=psh.orientation window) :landscape)
		 ...
		 (if (equal (ps=psh.orientation window) :landscape)
		   ...
		   (- (second (ps=psh.output.device.original.boundaries window))
		      (fourth (ps=psh.output.device.original.boundaries window))))))
	(t (pro-without.scheduling
	    (format win*output.stream "windowsize ~A~%" (win=window.tcl.name window)))
	   (force-output win*output.stream)
	   (multiple-value-bind (x y) (values-list (win=io.any.tyi (win=window.control.index window)))
...
(defun win-io.canvas.coordinates (window)
  ...
  (win-font.size 'roman 12)
  (pro-without.scheduling
   (format win*output.stream "canvassize ~A~%" (win=window.tcl.name window)))
  (force-output win*output.stream)
  (let ((result (win=io.any.tyi (win=window.control.index window))))
    (values 0 (car result) 0 (second result))))


(defun win-io.get.offset (window)

  ;;; Input:  a window of type \verb$WIN=WINDOW$
  ;;; Value:  return a multiple value, specifying the offset of the visible part of the window in pixel

  (declare (type win*window window))
  (pro-without.scheduling
   (format win*output.stream "windowoffset ~A~%" (win=window.tcl.name window)))
  (force-output win*output.stream)
  (let ((result (win=io.any.tyi (win=window.control.index window))))
    (values (floor (car result)) (floor (second result)))))
...
(defun win-font.size (font size)
  ...
  (let* ((window (win-window 'main)) result)
    ...
    (unwind-protect (progn (setq win*active t)
			   (pro-without.scheduling
			    (format win*output.stream "fontheight ~s ~A~%" 
				    (win=determine.font (cons font size))
				    (win=window.tcl.name (win-window 'main))))
			   (force-output win*output.stream)
			   (setq result (win=io.any.tyi (win=window.control.index window))))
      (setq win*active nil))
    (values-list result)))
...
(defun win-bitmap.size (bitmap)
  ...
  (let* (result bitmap.name)
    (setq bitmap.name (win=bitmap.name bitmap))
    (cond (win*active
	   (pro-process.wait "Requesting" #'(lambda () (not win*active)))))
    (unwind-protect (progn (setq win*active t)
			   (pro-without.scheduling
			    (format win*output.stream "bitmap_size ~A~%" 
				    bitmap.name (win=window.tcl.name (win-window 'main))))
			   (force-output win*output.stream)
			   (setq result (win=io.any.tyi (win=window.control.index (win-window 'main)))))
      (setq win*active nil))
    (values-list result)))


(defun win=bitmap.name (bitmap)

  (cond ((cassoc bitmap win*bitmaps :test 'equal))
	(t (let* (name (directory (append (pathname-directory *inka-pathname*) (list "tcl" "balls")))
		       (filename (namestring (make-pathname :directory directory :name bitmap :type "gif"))))
	     (cond (win*active
		    (pro-process.wait "Requesting" #'(lambda () (not win*active)))))
	     (unwind-protect (progn (setq win*active t)
				    (pro-without.scheduling
				     (format win*output.stream "bitmap_create ~s ~%"
					     filename (win=window.tcl.name (win-window 'main))))
				    (force-output win*output.stream)
				    (setq name (car (win=io.any.tyi 
						     (win=window.control.index (win-window 'main))))))
	       (setq win*active nil))
	     (push (cons bitmap name) win*bitmaps)
	     name))))
...
(defun win-io.string.width (font string)
  ...
  (let* ((window (win-window 'main)))
    ...
    (unwind-protect (progn (setq win*active t)
			   (pro-without.scheduling
			    (format win*output.stream "stringwidth ~s " (win=determine.font font))
			    (win=io.submit.string string)
			    (format win*output.stream " ~A~%" (win=window.tcl.name (win-window 'main))))
			   (force-output win*output.stream)
			   (win=io.any.tyi (win=window.control.index window)))
      (setq win*active nil))))
...
(defun win-io.shadow (window)
  ...
  (declare (type win*window window))
  (pro-without.scheduling
   (format win*output.stream "wm withdraw ~D~%" (win=window.tcl.name window))
   (format win*output.stream "update~%" (win=window.tcl.name window)))
  (force-output win*output.stream))
...
(defun win-io.expose (window)
  ...
  (declare (type win*window window))
  (pro-without.scheduling
   (format win*output.stream "wm deiconify ~D ~%" (win=window.tcl.name window))
   (format win*output.stream "update~%" (win=window.tcl.name window)))
  (force-output win*output.stream))
...
(defun win-io.clear (window)
  ...
  (declare (type win*window window))
  (pro-without.scheduling
   (format win*output.stream "~A.canvas.text delete 0.0 end ~%" (win=window.tcl.name window)))
  (force-output win*output.stream))
...
(defun win-io.clear.input (window)
  ...
  (declare (type win*window window))
  (setf (aref win*cursors (win=window.request.index window))
	(aref win*ends (win=window.request.index window))))
...
(DEFUN WIN-IO.CURSOR.WAIT (WINDOW)
  ...
  (declare (type win*window window))
  (pro-without.scheduling
   (format win*output.stream "~A configure -cursor watch~%" (win=window.tcl.name window)))
  (force-output win*output.stream))
...
(DEFUN WIN-IO.CURSOR.NORMAL (WINDOW)
  ...
  (declare (type win*window window))
  (pro-without.scheduling
   (format win*output.stream "~A configure -cursor top_left_arrow~%" (win=window.tcl.name window)))
  (force-output win*output.stream))
  ...
  (cond ((eq (type-of window) 'win*window)
	 (cond ((> (length string) 1000)
		(let ((cursor 0))
		  ...
		  (win-io.print "" window)))	       
	       (t (pro-without.scheduling
		   (format win*output.stream "~A.canvas.text insert end " (win=window.tcl.name window))
		   (win=io.submit.string string t)
		   (terpri win*output.stream))
		  (cond ((not win*forced.output.disabled) (force-output win*output.stream)))))
...
(defun win-io.print.top (window string)
  ...
  (cond ((eq (type-of window) 'ps*postscript.handler)
	...
	(t (pro-without.scheduling
	    (format win*output.stream "set ~A.toptext ~s~%" (win=window.tcl.name window) string))
	   (force-output win*output.stream))))
...
(defun win-io.print.status (window string)
  ...
  (declare (type win*window window))
  (pro-without.scheduling
   (format win*output.stream "set ~A.bottomtext ~s~%" (win=window.tcl.name window) string))
  (force-output win*output.stream))
...
(DEFUN WIN-IO.INITIALIZE.WINDOW (WINDOW WIDTH HEIGHT X-OFFSET Y-OFFSET)
  ...
  (pro-without.scheduling
   (format win*output.stream "initwin ~A ~D ~D ~D ~D~%" (win=window.tcl.name window) WIDTH HEIGHT X-OFFSET Y-OFFSET))
  (force-output win*output.stream))
...
(defun win-io.any.tyi (window)
  ...
  (declare (type win*window window))
  (win=io.any.tyi (win=window.request.index window)))
...
(defmacro win-io.listen (window)
  ...
  (not (eql (aref win*cursors (win=window.request.index window))
	    (aref win*ends (win=window.request.index window)))))
...
(defun win-window (type)
  ...
  (find-if #'(lambda (window)
	       (eq (win=window.name window) type))
	   win*window.all))
...
(defun win-stop (window)
  ...
  (pro-without.scheduling (format win*output.stream "exit~%"))
  (force-output win*output.stream)
  (finish-output win*output.stream))
...
(defun win-mn.popup (window entry.list)
  ...
  (declare (type win*window window))
  (pro-without.scheduling (win=mn.popup window (format nil "~A.dummy" (win=window.tcl.name window)) entry.list)
			  (format win*output.stream "popupmen ~A.dummy ~A~%"
				  (win=window.tcl.name window)
				  (win=window.tcl.name window)))
  (force-output win*output.stream)
  (prog1 (win=popup.get.result (win=io.any.tyi (win=window.control.index window)) entry.list)
    (format win*output.stream "destroy ~A.dummy~%" (win=window.tcl.name window))
    (force-output win*output.stream)))


(defun win=mn.popup (window men.name entry.list)

  (let ((counter 0) name)
    (format win*output.stream "menu ~A -tearoff 0 ~%" men.name)
    (mapc #'(lambda (entry)
	      (incf counter)
	      (cond ((consp (car entry))
		     (setq name (format nil "~A.men~D" men.name counter))
		     (format win*output.stream "~A add cascade -label {~A} -menu ~A~%" 
			     men.name (caar entry) name)
		     (win=mn.popup window name (cdar entry)))
		    (t (format win*output.stream "~A add command -label {~A} " men.name (car entry))
		       (format win*output.stream " -command {popputs {(~A c \"~A\")\}} ~%"
			       (win=window.tcl.name window) (car entry)))))
	  entry.list)))
...
(defun win=popup.get.result (string entry.list)
  (some #'(lambda (entry)
	    (cond ((consp (car entry))
		   (win=popup.get.result string (cdar entry)))
...
(defun win-mn.choose (window entry.list &optional (message "Choose from:"))
  ...
  (pro-without.scheduling
   (progn (mapc #'(lambda (entry)
		    (format win*output.stream ".listbox.m.select insert end ") 
		    (win=io.submit.string (car entry))
		    (format win*output.stream "~%"))
		entry.list)
	  (format win*output.stream "menu.choose ~A~%" (win=window.tcl.name window))))
  (force-output win*output.stream)
  (mapcar #'(lambda (no)
	      (declare (type fixnum no))
	      (cdr (nth  no entry.list)))
	  (win=io.any.tyi (win=window.control.index window))))
...
(defun win-text.input (window string &optional (string2 ""))
  ...
  (declare (type win*window window))
  (pro-without.scheduling
   (format win*output.stream "textinput ~A " (win=window.tcl.name window))
   (win=io.submit.string string)
   (format win*output.stream " ")
   (win=io.submit.string string2)
   (terpri win*output.stream))
  (force-output win*output.stream)
  (win=io.any.tyi (win=window.control.index window)))
...
(defun win-io.get.filename (fullname command)
  ...
  ;;;        and a string denoting either "proof","specification"
  ...
  ;;;        denoting a full file name, if fullname is specified partname becomes obsolete,
  ;;;        if \verb$FULLNAME$ is not specified, i.e. the empty string then the default path together
  ;;;        with the \verb$PARTNAME$ is the default

  (pro-without.scheduling
   (format win*output.stream "filename ~s ~s r~%" fullname 
	   (cond ((equalp command "proof") ".PRV")
		 ((equalp command "specification") ".SPEC"))))
  (force-output win*output.stream)
  (car (win=io.any.tyi (win=window.control.index (win-window 'main)))))
...
(defun win=colour.to.tcl (colour)

  (cond ((equal colour "DarkBlue")  "blue4")
	((equal colour "DarkGreen") "dark green")
	((equal colour "DarkRed") "red2")
	((equal colour "DarkYellow") "gold")
	((equal colour "DarkPink") "magenta")
	((equal colour "DarkGrey") "#999999")
	((equal colour "DarkGreyWhite") "#999999")
	((equal colour "LightBlue") "LightSkyBlue1")
	((equal colour "LightGreen") "green")
	((equal colour "LightRed") "LightSalmon2")
	((equal colour "LightYellow") "yellow")
	((equal colour "LightPink") "orchid1")
	((equal colour "LightGrey") "#E5E5E5")
	(t colour)))
